...
namespace Avalonia.Extensions.Controls
{
  public sealed class MessageBox : Window
  {
    ...
    public static Task<bool?> Show(Window parent, string title, string message, MessageBoxButtons messageBoxButtons = MessageBoxButtons.OkNo)
    {
      ...
      if (parent != null)
      {
        double x = parent.Position.X + parent.ActualWidth() / 2 - messageBox.Width/2;
        double y = parent.Position.Y + parent.ActualHeight() / 2- messageBox.Height/2;
        messageBox.Position = new PixelPoint((int)x, (int)y);
